...
public class PageConstructHero : MonoBehaviour {
    ...
    public GameObject backgroundContent, trump,joy;
    ...
    public GameObject butTrialSkis, butTrialJetpack, butTrialBoat;
    ...
    void LoadSkisChoose()
    {
        for (int i = 0; i < Modules.listSkisUse.Count; i++)
        {
            ...
            if (skisCon.idSkis == codeSkisChoose)
            {
                ...
                heroChooseShow.transform.eulerAngles = new Vector3(8, 160,-3);
                HeroController heroNow = heroChooseShow.GetComponent<HeroController>();
    ...
    public void ButtonCoinHeroClick()
    {
        ...
        if (!Modules.listHeroUnlock.Contains(codeHeroChoose))
        {
            ...
            if (Modules.totalCoin >= cost && CharcaterOnOFF.instance.ComingSoon == false)//neu du tien mua
            {
                playAudioCoin = true;
                ApllyTaskSpend(cost);
    ...
    {
        if (CharcaterOnOFF.instance.ComingSoon == false)
        {
